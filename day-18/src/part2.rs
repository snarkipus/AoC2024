...
mod graph {
    ...
    pub fn build_initial_graph(coords: &[Position]) -> miette::Result<(Graph, HashMap<(usize, usize), NodeIndex>)> {
    ...
    pub fn add_wall_to_graph(graph: &mut Graph, node_map: &HashMap<(usize, usize), NodeIndex>, pos: Position) -> miette::Result<()> {
    ...
    fn add_all_edges(graph: &mut Graph, grid: &Grid, node_map: &HashMap<(usize, usize), NodeIndex>) {
